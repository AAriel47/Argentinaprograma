...
public class Carrito {
	...
	static Path venta = Paths.get("ventas.txt");
	static Path productos = Paths.get("productos.txt");
		...
		while(salir2) {
			...
			switch (res=teclado.next().substring(0,1).toLowerCase()) {
				case "a": 
						...
						//try
							try {
								if(!Files.exists(productos)) {
									Files.createFile(productos);
								}else {
									for(String linea:Files.readAllLines(productos)) {
										cargar = linea;
									}
						...
						try {
							...
							if ("".equals(cargar)) {
								Files.writeString(productos, cargar);
							}else {
								cargar = cargar+" - "+resul;
								Files.writeString(productos,cargar);
							}
				...
				case "b": 
						...
						try {
							if(!Files.exists(productos)) {
								...
								String [] codigo1 = cargar.split(" ");
								//System.out.println(Arrays.toString(codigo1));
						...
						try {
							if(!Files.exists(venta)) {
								Files.createFile(venta);
							}else {
								for(String linea2:Files.readAllLines(venta)) {
									cargar2=linea2;
								}
							}
							if("".equals(cargar2)) {
								Files.writeString(venta, cargar2);
							}else {
				...
				case "c":
						...
						for (int e = 0; e<producto.length;e++) {
							...
							//System.out.println(e%(8*k)<1?true:false);
						}
						System.out.println(" ");
				...
				case "d":
					try {
						if(!Files.exists(venta)) {
							...
							for(String linea:Files.readAllLines(venta)) {
								cargar = linea;
							}
